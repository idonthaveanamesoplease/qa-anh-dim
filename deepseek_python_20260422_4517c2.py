...
def extract_dimensions_from_image(image):
    ...
    # Pattern: số + " (dấu nháy kép) hoặc số + "in" hoặc số + "W/H/D"
    patterns = [
        r'(\d+(?:\.\d+)?)\s*["\']',  # 54"
        r'(\d+(?:\.\d+)?)\s*in',      # 54 in
        r'(\d+(?:\.\d+)?)\s*"',       # 54"
        r'(\d+(?:\.\d+)?)\s*(?:W|H|D|x)' # 54 W
    ]
    ...
    # Lấy các số đầu tiên (thường là Width, Depth, Height)
    unique_numbers = list(dict.fromkeys(numbers))  # bỏ trùng
    
    return {
        "raw_text": full_text,
        "numbers": unique_numbers,
        "formatted": " x ".join([f"{n}\"" for n in unique_numbers[:3]])  # VD: 54" x 30" x 28"
    }
...
if excel_file and image_file:
    ...
    with col2:
        ...
        if idx is not None:
            ...
            # Lấy các cột dimension (từ cột D trở đi)
            dim_columns = df.columns[3:]  # cột D, E, F,...
            excel_dimensions = {}
            ...
            # So sánh (lấy dimension đầu tiên trong Excel để so sánh)
            st.markdown("### ✅ So sánh PASS/FAIL")
            
            # Lấy dimension đầu tiên (VD: Overall Dimension)
            ...
            if category:
                st.info(f"🏷️ Category xác định từ filename: **{category}**")
        
        else:
            st.error(f"❌ Không tìm thấy filename '{filename}' trong file Excel")
            st.write("Hãy kiểm tra lại cột B trong Excel có chứa tên này không")
